...
from click_package.tests.config import (
    STAT_OFFSET_GID,
    STAT_OFFSET_UID,
    STAT64_OFFSET_GID,
    STAT64_OFFSET_UID,
    STAT64T64_OFFSET_GID,
    STAT64T64_OFFSET_UID,
    )
...
class Stat64t64(ctypes.Structure):
    _pack_ = 1
    _fields_ = []
    _fields_.append(
        ("pad0", ctypes.c_ubyte * min(STAT64T64_OFFSET_UID, STAT64T64_OFFSET_GID)))
    if STAT64T64_OFFSET_UID < STAT64T64_OFFSET_GID:
        _fields_.append(("st_uid", ctypes.c_uint32))
        pad = (STAT64T64_OFFSET_GID - STAT64T64_OFFSET_UID -
               ctypes.sizeof(ctypes.c_uint32))
        assert pad >= 0
        if pad > 0:
            _fields_.append(("pad1", ctypes.c_ubyte * pad))
        ...
        _fields_.append(("st_gid", ctypes.c_uint32))
        pad = (STAT64T64_OFFSET_UID - STAT64T64_OFFSET_GID -
               ctypes.sizeof(ctypes.c_uint32))
        assert pad >= 0
        if pad > 0:
            _fields_.append(("pad1", ctypes.c_ubyte * pad))
